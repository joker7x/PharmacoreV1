...
import { getAllUsers, updateUserPermissions, updateGlobalConfig } from '../services/supabase.ts';
...
export const AdminView: React.FC<AdminViewProps> = ({ onBack, drugsCount, config, onUpdateConfig, userPoints, setPoints }) => {
  ...
  const saveConfig = async (updates: Partial<AdminConfig>) => {
    ...
    addLog(`تم تحديث إعدادات: ${Object.keys(updates).join(', ')}`);
    try {
  ...
  const handleManualPoints = (amount: number) => {
    if (!strictAuth) {
      alert("يرجى تفعيل وضع التحقق الصارم قبل تعديل النقاط!");
      return;
    }
    setPoints(amount);
    ...
    <div className="fixed inset-0 z-[200] bg-[#f8fafc] text-slate-900 pt-16 px-6 pb-40 overflow-y-auto no-scrollbar" dir="rtl">
      ...
      <div className="sticky top-0 z-50 bg-[#f8fafc]/90 backdrop-blur-md pb-6 pt-2">
        <div className="premium-card rounded-3xl p-1.5 flex items-center overflow-x-auto no-scrollbar">
          <TabButton id="dashboard" label="الرئيسية" icon={Activity} />
          <TabButton id="economy" label="الاقتصاد" icon={Coins} />
          <TabButton id="users" label="الأعضاء" icon={Users} />
          <TabButton id="system" label="النظام" icon={Terminal} />
        </div>
      </div>
          ...
          <MDiv key="dashboard" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="premium-card p-6 rounded-[32px] flex flex-col justify-between h-40">
                ...
                <div><div className="text-[10px] font-black text-slate-400 uppercase mb-1">المستخدمين</div><div className="text-3xl font-black">{users.length || '---'}</div></div>
              </div>
            </div>
            ...
            <div className="premium-card p-6 rounded-[32px] border-l-4 border-amber-500">
               <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="font-black text-sm">التدقيق الأمني المستمر</h3>
               </div>
               <div className="space-y-3">
                  ...
                  )) : <div className="text-center py-4 text-slate-300 font-bold text-xs uppercase tracking-widest">No Recent Logs</div>}
               </div>
            </div>
          </MDiv>
        )}
            ...
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-xl">
               <div className="flex justify-between items-center mb-10">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg font-black">إدارة النقاط (Strict)</h3>
                  </div>
                  <button onClick={() => { setStrictAuth(!strictAuth); addLog(`تغيير وضع التحقق: ${!strictAuth ? 'نشط' : 'متوقف'}`); }} className={`px-4 py-2 rounded-xl text-[10px] font-black flex items-center gap-2 transition-all ${strictAuth ? 'bg-emerald-500 text-white shadow-lg' : 'bg-slate-100 text-slate-400'}`}>
               ...
               <div className="grid grid-cols-2 gap-4 mb-8">
                  <div className="p-6 bg-slate-50 rounded-[32px] border border-slate-100">
                    <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block tracking-widest">نقاط المشاهدة</ts-span>
               ...
               <div className="space-y-4">
                  <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-100 rounded-2xl">
                    ...
                    <p className="text-[11px] font-bold text-blue-700 leading-relaxed">تعديل النقاط يدوي يتطلب تفعيل "وضع المحقق". جميع التغييرات يتم تسجيلها في السجل الأمني.</p>
                  </div>
        ...
        {activeTab === 'system' && (
          <MDiv key="system" initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
            ...
            <div className="premium-card p-8 rounded-[40px] bg-slate-900 text-white border-none shadow-2xl">
               ...
               <button onClick={() => { addLog("تم إرسال بث عام"); alert("تم إرسال الإشعار لجميع المستخدمين."); }} className="w-full py-5 bg-blue-600 rounded-[22px] font-black text-sm flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl shadow-blue-600/20"><Send size={18} /> إطلاق البث الآن</button>
            </div>
          </MDiv>
        )}
      </AnimatePresence>
...
const SystemAction = ({ label, description, icon: Icon, active, onClick }: any) => (
  <div className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-[28px]">
    ...
    <button onClick={onClick} className={`w-14 h-7 rounded-full p-1 transition-all flex items-center ${active ? 'bg-blue-600' : 'bg-slate-200'}`}>
      <motion.div animate={{ x: active ? -28 : 0 }} className="w-5 h-5 bg-white rounded-full shadow-md" />
    </button>
  </div>
);
